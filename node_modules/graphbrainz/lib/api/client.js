...
class ClientError extends _es6Error2.default {
...
class Client {
  constructor({
    baseURL,
    userAgent = `${_package2.default.name}/${_package2.default.version} ` + `( ${_package2.default.homepage || _package2.default.author.url || _package2.default.author.email} )`,
    extraHeaders = {},
    ...
    this.limiter = new _rateLimit2.default({ limit, period, concurrency });
    this.retryOptions = {
      retries,
  ...
  _get(path, options = {}, info = {}) {
    return new _promise2.default((resolve, reject) => {
      ...
      const req = (0, _request2.default)(options, (err, response, body) => {
        if (err) {
      ...
      debug(`Sending request. url=${req.uri.href} attempt=${info.currentAttempt}`);
    });
  }
  ...
  get(path, options = {}) {
    return new _promise2.default((resolve, reject) => {
      ...
      const operation = _retry2.default.operation(this.retryOptions);
      operation.attempt(currentAttempt => {
        ...
        this.limiter.enqueue(fn, [path, options, { currentAttempt }], priority).then(resolve).catch(err => {
